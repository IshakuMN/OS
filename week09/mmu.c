...
#define UINT_MAX 999999

unsigned int counters[MAX_PAGES];
#define TLB_SIZE 20
...
int random_page(struct PTE *pageTable, int numPages)
{
    return rand() % numPages;
}

int nfu(struct PTE *pageTable, int numPages)
{
    int victim = 0;
    ...
    for (int i = 0; i < numPages; i++)
    {
        if (pageTable[i].valid && counters[i] < minCounter)
        {
...
int aging(struct PTE *pageTable, int numPages)
{
    int victim = 0;
    int minCounter = UINT_MAX;
...
{
    counters[pageNum] = (counters[pageNum] >> 1) | (0x80);
}
